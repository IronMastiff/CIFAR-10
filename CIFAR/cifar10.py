...
#from six.moves import urllib      to make shure the model can be use in python2 and python3
...
LEARNING_PATE_DECAY_FACTOR = 0.1
...
def inference( images ):
    ...
    #pool1
    pool1 = tf.nn.max_pool( conv1, ksize = [1, 3, 3, 1], strides = [1, 2, 2, 1], padding = 'SAVME', name = 'pool1' )
    #norm1
    norm1 = tf.nn.lrn( pool1, 4, bias = 1.0, alpha = 0.001 / 9.0, beta = 0.75, name = 'norm1' )
    #conv2
    with tf.variable_scope( 'conv2' ) as scope:
        kernel = _variable_with_weight_decay( 'weights', shape = [5, 5, 64, 64], stddv = 1e-4, wd = 0.0 )
    ...
    #pool2
    pool2 = tf.nn.max_pool( norn2, ksize = [1, 3, 3, 1], strides = [ 1, 2, 2, 1], padding = 'SAME', name = 'pool2' )
    ...
    with tf.variable_scope('softmax_linear' )as scope:
        ...
        softmax_linear = tf.add( tf.matmul( local4, weights ), biases, naem = scope.name )
...
def loss( logits, labels ):
    ...
    dense_labels = tf.sparse_to_dense( concated,[FLAGS.batch_size, NUM_CLASSES, 1.0, 0.0] ) #sparse_to_dense one hot
    cross_entropy = tf.nn.softmax_cross_entropy_with_logits( logits, dense_labels, name = 'cross_entropy_per_example' )
    cross_entropy_mean = tf.reduce_mean( cross_entropy, name = 'cross_entropy' )
    tf.add_to_collection( 'losses', cross_enttropy_mean )
...
def _add_loss_summaries( total_loss ):
    ...
    for l in losses + [total_loss]:
        ...
        tf.ssummary.scalar( l.op.name, loss_averages.average(l) )
    return loss_averages_op

def train( total_loss, globel_step ):
    ...
    apply_gradient_op = opt.apply_gradients( grads, global_step = globel_step )
    for var in tf.trainable_variables():
        tf.summary.histogram( var.op.name, var )

    for grade, var in grads:
        if grad:
            tf.summary.histogram( var.op.name, var )

    variable_averages = tf.train.ExponentialMovingAverage(
        MOVING_AVERAGE_DECAY, globel_step )
...
def maybe_download_and_extract():
    ...
    if not os.path.exists( filepath ):
        def _progress( count, block_size, total_size ):
            ...
            sys.stdout.flush()
        filepath, _ = urllib.request.urlretrieve( DATA_URL, filepath, reprothook = _progress )
        print()
